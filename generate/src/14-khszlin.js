  ...
  return new Promise((resolve, reject) => {
    (async () => {
      try {
        ...
        report(khs, "Nepodařilo se z pdf vytvořit screenshot pro OCR");
        reject();
      }
    })();
  });
...
function generateOCRimage(crop, saveToFile) {
  sharpPromises.push(new Promise((resolve, reject) => {
    try {
      ...
      report(khs, "Nepodařilo se vygenerovat ořezy pro OCR");
      reject();
    }
  }));
}
...
function generateOCRjson() {
  return new Promise((resolve, reject) => {
    try {
      ...
      report(khs, "Nepodařilo se vytvořit json data.");
      reject();
    }
  });
}
  ...
  (async () => {
    ...
    if (checkFileExists && fs.existsSync(PDFfilePath)) {
      ...
      try {
        ...
        await page.goto('http://www.khszlin.cz/', {
          waitUntil: 'networkidle2'
        });

        // hledání url s pdf (každý den se URL mění)
        [url, crawledTime] = await page.evaluate(() => {
        ...
        report(khs, "Nepodařilo se získat URL adresu souboru nebo datum.")
        reject();
      }
    }

    if (crawledTime !== undefined) {
      try {
        // čištění času
        ...
        report(khs, "Nepodařilo se zpracovat datum");
        reject();
      }
    }
    ...
    if (url !== undefined) {
      try {
        ...
        report(khs, "Nepodařilo se uložit pdf.");
        reject();
      }
    }
  })();
